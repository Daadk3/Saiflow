...
export default function ForgotPasswordPage() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error || t('auth.forgotPasswordPage.errorGeneric'));
      } else {
        setSuccess(true);
      }
    } catch {
      setError(t('auth.forgotPasswordPage.errorGeneric'));
    } finally {
      setLoading(false);
    }
  }
          ...
          <div className="bg-[#111111] rounded-2xl border border-gray-800/50 shadow-2xl shadow-black/50 p-8">
            {success ? (
              ...
              <div className="text-center">
                ...
                <h1 className="text-2xl font-bold text-white mb-2">{t('auth.forgotPasswordPage.successTitle')}</h1>
                <p className="text-gray-400 mb-6">
                  {t('auth.forgotPasswordPage.successBody', { email })}
                </p>
                <p className="text-gray-500 text-sm mb-8">
                  {t('auth.forgotPasswordPage.successRetryHint')}
                </p>
                <div className="space-y-3">
                  ...
                  >
                    {t('auth.forgotPasswordPage.successTryAnother')}
                  </button>
                  <Link
                    href="/login"
              ...
              <>
                ...
                <div className="text-center mb-8">
                  ...
                  <h1 className="text-2xl font-bold text-white">{t('auth.forgotPasswordPage.heroTitle')}</h1>
                  <p className="mt-2 text-gray-500">{t('auth.forgotPasswordPage.heroSubtitle')}</p>
                </div>
